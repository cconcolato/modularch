...
MP4_Simple* MP4_Simple::create(const std::string &fn) {
	...
	if ((e != GF_OK && e != GF_ISOM_INCOMPLETE_FILE) || movie == NULL) {
		Log::get(ModulArch::Log::Error) << "Could not open file " << fn << " for reading (" << gf_error_to_string(e) << ")." << std::endl;
		return NULL;
	}

	return new MP4_Simple(movie);
}

void MP4_Simple::deleteLastSample() {
	if (iso_sample) {
...
MP4_Simple::MP4_Simple(GF_ISOFile *movie)
  : movie(movie), iso_sample(NULL) {
	...
	if (track_number == 0) {
		Log::get(ModulArch::Log::Error) << "Could not find track ID=" << track_id << std::endl;
	}
...
std::vector<char*>& MP4_Simple::process(std::vector<char*> &in) {
	...
	if (iso_sample) {
		Log::get(ModulArch::Log::Error) << "Found sample #" << sample_index << "/" << sample_count << " of length " << iso_sample->dataLength << ", RAP: " << iso_sample->IsRAP << ", DTS: " << iso_sample->DTS << ", CTS: " << iso_sample->DTS+iso_sample->CTS_Offset << std::endl;
		...
		if (e == GF_ISOM_INCOMPLETE_FILE) {
			...
			Log::get(ModulArch::Log::Error) << "Missing " << missing_bytes << " bytes on input file" << std::endl;
